...
- Match traffic goes directly between the two players where their networks allow it; rooms, matchmaking and compatibility stay on the server. That removes the relay hop from the round trip, which is what the automatic input delay is sized from. Pairs that cannot reach each other fall back to the relay automatically, and `MELEE_NETPLAY_RELAY_ONLY=1` keeps everything on the server.
...
- Options includes **Controller** (mapping, input visualization, tap jump and per-port trigger settings: where the L/R click engages, light shield and trigger dead zone) and **Widescreen**, with 4:3 and 16:9 choices.
...
- Presentation follows the display. The simulation always runs at the GameCube's 59.94 Hz, so timing and game speed are unchanged; on a monitor faster than 60 Hz the finished frames are shown in step with it, which removes tearing at no cost, and a variable-refresh monitor then simply refreshes at 59.94. A 60 Hz monitor keeps the unsynchronised path, where a frame that runs a millisecond long costs a millisecond instead of a whole refresh. `GCN_AURORA_VSYNC` forces either behaviour.
- **Play YAMPP.cmd** checks whether the master server is running a newer release and updates from this repository before starting. A room only admits clients on a matching build, so being out of date means not being able to play online at all. The server reports a version and nothing else -- the download address is pinned in the launcher -- and the archive is checked against its published checksum before anything is unpacked. Your settings, saves, extracted disc and imported content are never replaced. `YAMPP_NO_UPDATE=1` skips the check.
