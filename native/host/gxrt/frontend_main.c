...
int main(int argc, char** argv)
{
     ...
     * with DI reads still at zero. */
    {
        ...
        if (ff) {
             ...
             * for it was enough to make the item switch screen fail its
             * first allocation and panic.
             *
             * Measured peak across the whole menu set, Online included, is
             * 69,152 bytes; the Online labels are most of it, so a figure
             * taken before that screen is opened badly understates it.
             * 96 KB leaves about 40% headroom, and area_alloc warns rather
             * than failing silently if that is ever not enough. */
            u32 host_area = (boot_thread - 0x18000u) & ~31u;
            { extern void hooks_init(Context*, uint32_t, uint32_t); hooks_init(&s_cpu, host_area, 0x18000u); }
